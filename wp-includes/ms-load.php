 ...
 * manually, define `WP_PLUGIN_DIR` and `WP_PLUGIN_URL` in `my-portfolio-config.php`.
 *
 * @access private
 * @since 3.1.0
 *
...
function ms_load_current_site_and_network( $domain, $path, $subdomain = false ) {
	...
	// If the network is defined in my-portfolio-config.php, we can simply use that.
	...
	if ( empty( $current_blog ) ) {
		...
		if ( $subdomain && ! defined( 'NOBLOGREDIRECT' ) ) {
			...
			$destination .= 'my-portfolio-signup.php?new=' . str_replace( '.' . $current_site->domain, '', $domain );
		} elseif ( $subdomain ) {
			/*
