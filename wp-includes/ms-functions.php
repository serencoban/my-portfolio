...
function wpmu_signup_blog_notification(
	...
	if ( ! is_subdomain_install() || get_current_network_id() !== 1 ) {
		$activate_url = network_site_url( "my-portfolio-activate.php?key=$key" );
	} else {
		$activate_url = "http://{$domain}{$path}my-portfolio-activate.php?key=$key"; // @todo Use *_url() API.
	}
...
function wpmu_signup_user_notification(
	...
	$message         = sprintf(
		...
		site_url( "my-portfolio-activate.php?key=$key" )
	);
...
function signup_nonce_check( $result ) {
	if ( ! strpos( $_SERVER['PHP_SELF'], 'my-portfolio-signup.php' ) ) {
		return $result;
	}
