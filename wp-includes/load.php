 ...
 *
 * Development mode can be set via the `WP_DEVELOPMENT_MODE` constant in `my-portfolio-config.php`.
 ...
 * All three can be defined in my-portfolio-config.php. By default, `WP_DEBUG` and
...
function wp_debug_mode() {
	 ...
	 * WordPress loads, usually in `my-portfolio-config.php`.
	 *
	 * Example:
	 *
...
function wp_set_wpdb_vars() {
	...
	if ( is_wp_error( $prefix ) ) {
		...
		wp_die(
			sprintf(
				/* translators: 1: $table_prefix, 2: my-portfolio-config.php */
...
function wp_get_active_and_valid_themes() {
	...
	if ( wp_installing() && 'my-portfolio-activate.php' !== $pagenow ) {
		return $themes;
	}
...
function is_protected_endpoint() {
	// Protect login pages.
	if ( isset( $GLOBALS['pagenow'] ) && 'my-portfolio-login.php' === $GLOBALS['pagenow'] ) {
		return true;
	}
...
function wp_is_site_protected_by_basic_auth( $context = '' ) {
	...
	if ( ! $context ) {
		if ( 'my-portfolio-login.php' === $pagenow ) {
			$context = 'login';
		} elseif ( is_admin() ) {
			$context = 'admin';
		} else {
