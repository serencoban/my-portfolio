...
require_once dirname(__DIR__, 2) . '/my-portfolio-load.php';
...
if ( ! defined( 'WP_ALLOW_REPAIR' ) || ! WP_ALLOW_REPAIR ) {
	...
	$default_keys    = array_unique(
		array(
			...
			/*
			 * translators: This string should only be translated if my-portfolio-config-sample.php is localized.
			 * You can check the localized release package or
			 * https://i18n.svn.wordpress.org/<locale code>/branches/<wp version>/dist/my-portfolio-config-sample.php
			 */
	...
	if ( $duplicated_keys || $missing_key ) {
		...
		/* translators: 1: my-portfolio-config.php, 2: Secret key service URL. */
		echo '<p>' . sprintf( __( 'While you are editing your %1$s file, take a moment to make sure you have all 8 keys and that they are unique. You can generate these using the <a href="%2$s">WordPress.org secret key service</a>.' ), '<code>my-portfolio-config.php</code>', 'https://api.wordpress.org/secret-key/1.1/salt/' ) . '</p>';
	}
	...
	if ( $problems ) {
		...
		echo '<p>' . __( 'Repairs complete. Please remove the following line from my-portfolio-config.php to prevent this page from being used by unauthorized users.' ) . "</p><p><code>define('WP_ALLOW_REPAIR', true);</code></p>";
	}
} else {
