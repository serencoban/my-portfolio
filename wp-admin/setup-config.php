<?php
/**
 * Retrieves and creates the my-portfolio-config.php file.
 *
 * The permissions for the base directory must allow for writing files in order
 * for the my-portfolio-config.php to be created using this page.
 *
 * @package WordPress
...
require ABSPATH . 'my-portfolio-settings.php';
...
// Support my-portfolio-config-sample.php one level up, for the develop repo.
if ( file_exists( ABSPATH . 'my-portfolio-config-sample.php' ) ) {
	$config_file = file( ABSPATH . 'my-portfolio-config-sample.php' );
} elseif ( file_exists( dirname( ABSPATH ) . '/my-portfolio-config-sample.php' ) ) {
	$config_file = file( dirname( ABSPATH ) . '/my-portfolio-config-sample.php' );
} else {
	wp_die(
		sprintf(
			/* translators: %s: my-portfolio-config-sample.php */
			__( 'Sorry, I need a %s file to work from. Please re-upload this file to your WordPress installation.' ),
			'<code>my-portfolio-config-sample.php</code>'
		)
	);
}

// Check if my-portfolio-config.php has been created.
if ( file_exists( ABSPATH . 'my-portfolio-config.php' ) ) {
	wp_die(
		'<p>' . sprintf(
			/* translators: 1: my-portfolio-config.php, 2: install.php */
			...
			'<code>my-portfolio-config.php</code>',
			'install.php'
		) . '</p>',
		409
	);
...
// Check if my-portfolio-config.php exists above the root directory but is not part of another installation.
if ( @file_exists( ABSPATH . '../my-portfolio-config.php' ) && ! @file_exists( ABSPATH . '../my-portfolio-settings.php' ) ) {
	wp_die(
		'<p>' . sprintf(
			/* translators: 1: my-portfolio-config.php, 2: install.php */
...
/**
 * Display setup my-portfolio-config.php file header.
 *
 * @ignore
 * @since 2.3.0
 *
...
<p>
		<?php
		printf(
			/* translators: %s: my-portfolio-config.php */
	...
	<strong>
		<?php
		printf(
			/* translators: 1: my-portfolio-config-sample.php, 2: my-portfolio-config.php */
		...
		<?php
		printf(
			/* translators: 1: Documentation URL, 2: my-portfolio-config.php */
...
<p>
			<?php
			/* translators: %s: my-portfolio-config.php */
			printf( __( 'Unable to write to %s file.' ), '<code>my-portfolio-config.php</code>' );
			?>
</p>
<p id="wp-config-description">
			<?php
			/* translators: %s: my-portfolio-config.php */
			printf( __( 'You can create the %s file manually and paste the following text into it.' ), '<code>my-portfolio-config.php</code>' );
...
<p class="configuration-rules-label"><label for="wp-config">
			<?php
			/* translators: %s: my-portfolio-config.php */
			printf( __( 'Configuration rules for %s:' ), '<code>my-portfolio-config.php</code>' );
			?>
	</label></p>
...
if ( ! /iPad|iPod|iPhone/.test( navigator.userAgent ) ) {
	...
	el.select();
			...
			/*
			 * If this file doesn't exist, then we are using the my-portfolio-config-sample.php
			 * file one level up, which is for the develop repo.
			 */
			if ( file_exists( ABSPATH . 'my-portfolio-config-sample.php' ) ) {
				$path_to_wp_config = ABSPATH . 'my-portfolio-config.php';
			} else {
				$path_to_wp_config = dirname( ABSPATH ) . '/my-portfolio-config.php';
			}
			...
			if ( false !== $handle ) {
				...
				if ( ! empty( $wp_config_perms ) && ! is_writable( $path_to_wp_config ) ) {
					$error_message = sprintf(
						/* translators: 1: my-portfolio-config.php, 2: Documentation URL. */
