...
function network_step2( $errors = false ) {
	...
	if ( ! file_exists( ABSPATH . 'my-portfolio-config.php' ) && file_exists( dirname( ABSPATH ) . '/my-portfolio-config.php' ) ) {
	...
	if ( $_POST || ! is_multisite() ) {
		...
		if ( file_exists( $home_path . '.htaccess' ) ) {
			$notice_message .= sprintf(
				/* translators: 1: my-portfolio-config.php, 2: .htaccess */
			...
			$notice_message .= sprintf(
				/* translators: 1: my-portfolio-config.php, 2: web.config */
				__( 'You should back up your existing %1$s and %2$s files.' ),
				'<code>my-portfolio-config.php</code>',
	...
	<ol>
		...
		<?php
		printf(
			/* translators: 1: my-portfolio-config.php, 2: Location of wp-config file, 3: Translated version of "That's all, stop editing! Happy publishing." */
		...
		<p class="configuration-rules-label"><label for="network-wpconfig-rules">
			<?php
			printf(
				/* translators: %s: File name (my-portfolio-config.php, .htaccess or web.config). */
